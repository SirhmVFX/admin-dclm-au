...
export default function TeachingsPage() {
    ...
    const activeCategoryId = subCategories.find((s) => s.id === form.subCategoryId)?.categoryId ?? "";
    ...
    function openEdit(t: Teaching) {
        ...
        setForm({
            ...
            teacher: t.teacher, bibleVerse: t.bibleVerse, date: t.date,
            imgSrc: t.imgSrc, published: t.published,
    ...
    function addYoutubeLink() {
        setForm((p) => ({ ...p, youtubeLinks: [...p.youtubeLinks, { title: "", url: "" }] }));
    }
    function updateYoutubeLink(i: number, field: keyof YoutubeLink, value: string) {
        setForm((p) => ({ ...p, youtubeLinks: p.youtubeLinks.map((l, idx) => idx === i ? { ...l, [field]: value } : l) }));
    }
    function removeYoutubeLink(i: number) {
        setForm((p) => ({ ...p, youtubeLinks: p.youtubeLinks.filter((_, idx) => idx !== i) }));
    }

    async function handleSave() {
        if (!form.title) { setError("Title is required."); return; }
        if (form.youtubeLinks.some((l) => !l.url.trim())) { setError("All YouTube links must have a URL."); return; }
    ...
    return (
        <div className="max-w-5xl space-y-4">
            <div className="section-header">
                ...
                <button className="btn-primary flex items-center gap-2" onClick={openNew}><MdAdd size={16} /> New Teaching</button>
            </div>

            {loading ? <div className="admin-card text-sm text-gray-500">Loading…</div>
                : items.length === 0 ? <div className="admin-card text-sm text-gray-500 text-center py-8">No teachings yet.</div>
                    : (
                                        ...
                                        <tr key={t.id}>
                                            ...
                                            <td>
                                                {(t.youtubeLinks?.length ?? 0) > 0
                                                    ? <span className="flex items-center gap-1 text-xs text-red-500 font-medium"><MdVideoLibrary size={13} />{t.youtubeLinks.length}</span>
                    ...
                    <div className="modal-box" style={{ maxWidth: 780 }}>
                        ...
                        <div className="p-5 space-y-4">
                            ...
                            <div><label className="admin-label">Title</label>
                                <input className="admin-input" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="e.g. 1st Thessalonians" /></div>

                            <div><label className="admin-label">Short Description</label>
                                <textarea className="admin-input" rows={2} value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} /></div>

                            <ImageUpload value={form.imgSrc} onChange={(url) => setForm({ ...form, imgSrc: url })} label="Cover Image" />

                            <div className="grid grid-cols-2 gap-4">
                                <div><label className="admin-label">Teacher</label>
                                    <input className="admin-input" value={form.teacher} onChange={(e) => setForm({ ...form, teacher: e.target.value })} /></div>
                                <div><label className="admin-label">Bible Verse</label>
                                    <input className="admin-input" value={form.bibleVerse} onChange={(e) => setForm({ ...form, bibleVerse: e.target.value })} placeholder="e.g. 1 Thessalonians 5:23" /></div>
                                <div><label className="admin-label">Date</label>
                                    <input className="admin-input" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} /></div>
                            </div>
                                ...
                                <div className="space-y-3">
                                    <div>
                                        ...
                                        <div className="flex flex-wrap gap-2 mt-1">
                                            <label className={`flex items-center gap-1.5 text-sm px-3 py-1 border cursor-pointer transition-colors ${!form.subCategoryId ? "border-blue-500 bg-blue-50 text-blue-700" : "border-gray-200 text-gray-600 hover:border-gray-400"
                                                }`}>
                                                <input type="radio" className="hidden" checked={!form.subCategoryId}
                                                    onChange={() => setForm({ ...form, subCategoryId: "" })} />
                                                None
                                            </label>
                                            {categories.map((cat) => {
                                                const catSubs = subCategories.filter((s) => s.categoryId === cat.id);
                                                const selected = catSubs.some((s) => s.id === form.subCategoryId);
                                                return (
                                                    <label key={cat.id} className={`flex items-center gap-1.5 text-sm px-3 py-1 border cursor-pointer transition-colors ${selected ? "border-blue-500 bg-blue-50 text-blue-700" : "border-gray-200 text-gray-600 hover:border-gray-400"
                                                        }`}>
                                                        <input type="radio" className="hidden" checked={selected}
                                                            onChange={() => {
                                                                // auto-select first subcategory of this category
                                                                const first = catSubs[0];
                                                                setForm({ ...form, subCategoryId: first?.id ?? "" });
                                                            }} />
                                                        {cat.name}
                                                    </label>
                                                );
                                    ...
                                    {form.subCategoryId && (() => {
                                        const selectedCatId = subCategories.find((s) => s.id === form.subCategoryId)?.categoryId;
                                        const subs = subCategories.filter((s) => s.categoryId === selectedCatId);
                                        return subs.length > 0 ? (
                                            <div>
                                                <label className="admin-label">Subcategory</label>
                                                <div className="flex flex-wrap gap-2 mt-1">
                                                    {subs.map((sub) => (
                                                        <label key={sub.id} className={`flex items-center gap-1.5 text-sm px-3 py-1 border cursor-pointer transition-colors ${form.subCategoryId === sub.id ? "border-blue-500 bg-blue-50 text-blue-700" : "border-gray-200 text-gray-600 hover:border-gray-400"
                                                            }`}>
                                                            <input type="radio" className="hidden" checked={form.subCategoryId === sub.id}
                                                                onChange={() => setForm({ ...form, subCategoryId: sub.id! })} />
                                                            {sub.name}
                                                        </label>
                                                    ))}
                                                </div>
                            ...
                            <div>
                                <div className="flex items-center justify-between mb-2">
                                    ...
                                    <button type="button" className="btn-secondary py-1 px-2 text-xs flex items-center gap-1" onClick={addYoutubeLink}>
                                            ...
                                            <div key={i} className="border border-gray-200 p-3 space-y-2 bg-gray-50">
                                                <div className="flex items-center justify-between">
                                                    <span className="text-xs font-medium text-gray-600 flex items-center gap-1">
                                                        <MdVideoLibrary size={13} className="text-red-500" /> Recording {i + 1}
                                                    </span>
                                                    <button type="button" onClick={() => removeYoutubeLink(i)} className="text-gray-400 hover:text-red-500 transition-colors"><MdClose size={15} /></button>
                                                </div>
                                                <input className="admin-input" placeholder="Title (e.g. Session 1 — Introduction)" value={link.title} onChange={(e) => updateYoutubeLink(i, "title", e.target.value)} />
                                                <div className="flex items-center gap-2">
                                                    <MdLink size={14} className="text-gray-400 shrink-0" />
                                                    <input className="admin-input" placeholder="YouTube URL" value={link.url} onChange={(e) => updateYoutubeLink(i, "url", e.target.value)} />
                                                </div>
                                                {extractYoutubeId(link.url) && (
                                                    <div className="flex items-center gap-2 mt-1">
                                                        <Image src={`https://img.youtube.com/vi/${extractYoutubeId(link.url)}/mqdefault.jpg`} alt="thumbnail" width={120} height={68} className="w-28 h-16 object-cover border border-gray-200" />
                            ...
                            <div><label className="admin-label">Full Teaching Content</label>
                                <WysiwygEditor content={form.content} onChange={(html) => setForm({ ...form, content: html })} placeholder="Write the full teaching here…" /></div>
